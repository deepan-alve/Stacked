...
function gitBackup() {
  ...
  try {
    ...
    if (!status.trim()) {
      console.log(`[${new Date().toISOString()}] No database changes to commit`);
    ...
    try {
      ...
      console.log(`[${new Date().toISOString()}] ✓ Database backed up and pushed to Git`);
      return { success: true, message: "Committed and pushed" };
    } catch (pushError) {
      console.log(`[${new Date().toISOString()}] ✓ Database committed (push failed - will retry later)`);
    ...
    console.error(`[${new Date().toISOString()}] ✗ Git backup failed:`, error.message);
